...
<body>
	...
	<div id="sidebar">
		<ul>
			...
			<li>
				<a href="<?php echo site_url('User/nilai') ?>">
					<i class="icon icon-file"></i>
					<span>Daftar Nilai Siswa</span>
				</a>
			</li>
			<li>
				<a href="<?php echo site_url('Klaster') ?>">
	...
	<div id="content">
		...
		<div class="container-fluid">
			<h3>Pengelompokan Nilai Siswa Tahun <?php echo $thn ?> Iterasi Ke-1</h3>
							...
							<table class="table table-bordered data-table">
								...
								<tbody>
											...
											<?php foreach ($nilai_rand as $k) { ?>
												<td colspan="17">
													<!-- <?php
															...
															?> -->
													<?php
													// $mapai = max(array($cc_PAI[$e]));
													// $mipai = max(array($cc_PAI[$e])) - min(array($cc_PAI[$e]));
													// $hm[$e] = $key->PAI - $c_PAI[$e] / $mipai;
									...
									<tr align="center">
										...
										<td colspan="19"><b><?php echo $tc0 ?></b></td>
									</tr>
								</tbody>
							</table>
						</div>
					...
					<div class="widget-content nopadding">
						...
						<div style="overflow-x: auto;">
							<table class="table table-bordered">
								...
								<tbody>
									<?php $nom = 1 ?>
									<?php foreach ($nilai_rand2 as $nm1) { ?>
										<tr>
											<td><?php echo $nom ?></td>
											<td><?php echo $nm1->nama_siswa ?></td>
											<td><?php echo $nm1->nama_kelas ?></td>
											<td><?php echo $nm1->PAI ?></td>
											<td><?php echo $nm1->PAK ?></td>
											<td><?php echo $nm1->PPKn ?></td>
											<td><?php echo $nm1->BI ?></td>
											<td><?php echo $nm1->M ?></td>
											<td><?php echo $nm1->SI ?></td>
											<td><?php echo $nm1->Bing ?></td>
											<td><?php echo $nm1->SB ?></td>
											<td><?php echo $nm1->PJOK ?></td>
											<td><?php echo $nm1->F ?></td>
											<td><?php echo $nm1->K ?></td>
											<td><?php echo $nm1->SKD ?></td>
											<td><?php echo $nm1->SK ?></td>
											<td><?php echo $nm1->KJD ?></td>
											<td><?php echo $nm1->PD ?></td>
											<td><?php echo $nm1->DDG ?></td>
											<td><?php echo $nm1->BD ?></td>
							...
							<table class="table table-bordered">
								<thead>
									<tr>
										...
										<th rowspan="2">Tahun</th>
										<?php $f = 1 ?>
										<?php foreach ($nilai_rand2 as $m) { ?>
											<th colspan="17">Centroid <?php echo $f;
																		$f++ ?></th>
										<?php } ?>
										<?php $g = 1 ?>
										<?php foreach ($nilai_rand2 as $m) { ?>
									...
									<tr>
										<?php foreach ($nilai_rand2 as $nm1) { ?>
											<th><?php $cn_PAI[] = $nm1->PAI;
												echo $nm1->PAI ?></th>
											<th><?php $cn_PAK[] = $nm1->PAK;
												echo $nm1->PAK ?></th>
											<th><?php $cn_PPKn[] = $nm1->PPKn;
												echo $nm1->PPKn ?></th>
											<th><?php $cn_BI[] = $nm1->BI;
												echo $nm1->BI ?></th>
											<th><?php $cn_M[] = $nm1->M;
											...
											<th><?php $cn_SI[] = $nm1->SI;
												echo $nm1->SI ?></th>
											<th><?php $cn_Bing[] = $nm1->Bing;
												echo $nm1->Bing ?></th>
											<th><?php $cn_SB[] = $nm1->SB;
												echo $nm1->SB ?></th>
											<th><?php $cn_PJOK[] = $nm1->PJOK;
												echo $m1->PJOK ?></th>
											<th><?php $cn_F[] = $nm1->F;
												echo $nm1->F ?></th>
											<th><?php $cn_K[] = $nm1->K;
												echo $nm1->K ?></th>
											<th><?php $cn_SKD[] = $nm1->SKD;
												echo $nm1->SKD ?></th>
											<th><?php $cn_SK[] = $nm1->SK;
												echo $nm1->SK ?></th>
											<th><?php $cn_PD[] = $nm1->PD;
												echo $nm1->PD ?></th>
											<th><?php $cn_KJD[] = $nm1->KJD;
												echo $nm1->KJD ?></th>
											<th><?php $cn_DDG[] = $nm1->DDG;
												echo $nm1->DDG ?></th>
											<th><?php $cn_BD[] = $nm1->BD;
												echo $nm1->BD ?></th>
										<?php } ?>
								...
								<tbody>
									...
									<?php foreach ($nilai as $key) { ?>
										<tr>
											...
											<?php foreach ($nilai_rand2 as $k) { ?>
												<td colspan="17">
													<?php
													$hnm[$l] = sqrt(pow(($key->PAI - $cn_PAI[$l]), 2) + pow(($key->PAK - $cn_PAK[$l]), 2) + pow(($key->PPKn - $cn_PPKn[$l]), 2) + pow(($key->BI - $cn_BI[$l]), 2) + pow(($key->M - $cn_M[$l]), 2) + pow(($key->SI - $cn_SI[$l]), 2) + pow(($key->Bing - $cn_Bing[$l]), 2) + pow(($key->SB - $cn_SB[$l]), 2) + pow(($key->PJOK - $cn_PJOK[$l]), 2) + pow(($key->F - $cn_F[$l]), 2) + pow(($key->K - $cn_K[$l]), 2) + pow(($key->SKD - $cn_SKD[$l]), 2) + pow(($key->SK - $cn_SK[$l]), 2) + pow(($key->KJD - $cn_KJD[$l]), 2) + pow(($key->PD - $cn_PD[$l]), 2) + pow(($key->DDG - $cn_DDG[$l]), 2) + pow(($key->BD - $cn_BD[$l]), 2));
													echo $hnm[$l];
													$hcnm[$l] = $hnm[$l];
													?>
												</td>
						...
						<div class="jumbotron">
							<div class="container">
								<h4>
									...
									<?php
									$n = "insert into hasil_iterasi(iterasi,total_medoids,total_non_medoids,selisih) values('1','" . $tc0 . "','" . $tcnm0 . "','" . $selisih . "')";
									$this->db->query($n);
									?>
								</h4>
							</div>
